...
void BattleGroundBG::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    ...
    for (uint8 node = 0; node < BG_BG_NODES_MAX; ++node)
        FillInitialWorldState(data, count, BG_BG_OP_NODEICONS[node], m_Nodes[node] == 0);
    ...
    FillInitialWorldState(data, count, BG_BG_OP_OCCUPIED_BASES_ALLY, ally);
    FillInitialWorldState(data, count, BG_BG_OP_OCCUPIED_BASES_HORDE, horde);

    // Team scores
    FillInitialWorldState(data, count, BG_BG_OP_RESOURCES_MAX,      BG_BG_MAX_TEAM_SCORE);
    FillInitialWorldState(data, count, BG_BG_OP_RESOURCES_WARNING,  BG_BG_WARNING_NEAR_VICTORY_SCORE);
    FillInitialWorldState(data, count, BG_BG_OP_RESOURCES_ALLY,     m_TeamScores[BG_TEAM_ALLIANCE]);
    FillInitialWorldState(data, count, BG_BG_OP_RESOURCES_HORDE,    m_TeamScores[BG_TEAM_HORDE]);
