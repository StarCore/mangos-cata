...
void BattleGroundTP::EventPlayerCapturedFlag(Player* source)
{
    ...
    if (source->GetTeam() == ALLIANCE)
    {
        ...
        if (m_TeamScores[BG_TEAM_ALLIANCE] < BG_TP_MAX_TEAM_SCORE)
    ...
    else
    {
        ...
        if (m_TeamScores[BG_TEAM_HORDE] < BG_TP_MAX_TEAM_SCORE)
    ...
    if (m_TeamScores[BG_TEAM_ALLIANCE] == BG_TP_MAX_TEAM_SCORE)
        winner = ALLIANCE;
    else if (m_TeamScores[BG_TEAM_HORDE] == BG_TP_MAX_TEAM_SCORE)
        winner = HORDE;
...
{
    if (team == ALLIANCE)
        UpdateWorldState(BG_TP_FLAG_CAPTURES_ALLIANCE, m_TeamScores[BG_TEAM_ALLIANCE]);
    else
        UpdateWorldState(BG_TP_FLAG_CAPTURES_HORDE, m_TeamScores[BG_TEAM_HORDE]);
}
...
{
    FillInitialWorldState(data, count, BG_TP_FLAG_CAPTURES_ALLIANCE, m_TeamScores[BG_TEAM_ALLIANCE]);
    FillInitialWorldState(data, count, BG_TP_FLAG_CAPTURES_HORDE, m_TeamScores[BG_TEAM_HORDE]);
